...
data class BlockToPlace(
    ...
    // Возвращает базовый ингредиент, необходимый для крафта/установки этого блока
    val material: Material get() = blockData.material.toBaseIngredient()
}

/**
 * Функция расширения для группировки сложных блоков Minecraft в базовые материалы.
 */
fun Material.toBaseIngredient(): Material {
    val name = this.name

    val base = when {
        // --- 1. ДЕРЕВЯННЫЕ МАТЕРИАЛЫ (Все типы изделий сводятся к доскам своей породы) ---
        ...
        // --- 2. КАМЕННЫЕ МАТЕРИАЛЫ (Любые виды камня, кирпичей, сланца, базальта и т.д. -> COBBLESTONE) ---
        name.contains("STONE") ||
        ...
        // --- 4. КИРПИЧИ И ПРОЧИЕ СЛОЖНЫЕ БЛОКИ ---
        ...
        // --- 7. БЛОКИ-СИРОТЫ И ТЕХНИЧЕСКИЕ БЛОКИ ---
    ...
    // ЖЕСТКАЯ ЗАЩИТА: Если после всех преобразований материал всё ещё не является предметом
    // (например, технический портал), заменяем его на булыжник, чтобы никогда не крашить сервер.
