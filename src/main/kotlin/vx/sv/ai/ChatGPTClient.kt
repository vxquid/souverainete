...
class ChatGPTClient(
    ...
    private val client = OkHttpClient.Builder()
        .readTimeout(60, TimeUnit.SECONDS) // OpenAI иногда отвечает дольше, чем Groq
    ...
    override fun <T : Any> sendPromptWithSchema(
        ...
        // OpenAI требует наличия слова "JSON" в промпте для работы response_format: json_object (оно есть выше)
    ...
    private fun translateWithRetry(
        ...
        val parser = AdvancedJsonParser()
        val escapedPrompt = parser.escapeJsonString(prompt)
        val requestBodyJson = parser.createJsonRequest(escapedPrompt, false) // jsonMode = false для обычного текста
        val requestBody = requestBodyJson.toRequestBody("application/json".toMediaTypeOrNull())
    ...
    private inner class AdvancedJsonParser {
        ...
        fun <T : Any> parseResponse(content: String, responseType: Class<T>): T {
            ...
            // OpenAI возвращает ответ в той же структуре: choices[0].message.content
            var cleanedContent = jsonResponse
                ...
                ?.let { cleanJson(it) }
                ?: throw JsonParseException("Failed to extract JSON content from OpenAI response!")
        ...
        fun extractTextFromResponse(content: String): String {
            ...
            return jsonResponse
                ...
                ?.getAsJsonObject("message")
                ?.get("content")?.asString
                ?: throw JsonParseException("Failed to extract text from OpenAI response!")
        }
        ...
        fun createJsonRequest(prompt: String, jsonMode: Boolean): String {
            // OpenAI поддерживает response_format: { "type": "json_object" } для моделей gpt-4o, gpt-4-turbo, gpt-3.5-turbo
    ...
    private fun handleFailedResponse(response: Response, key: KeyManager.Key) {
        ...
        // OpenAI возвращает 429 при лимитах и ошибку insufficient_quota
        if (response.code == 429 || body.contains("quota") || body.contains("rate limit") || body.contains("insufficient_quota")) {
            key.quota = true
            plugin.logger.info("OpenAI Quota/Rate Limit exceeded. Resetting key usage check in 60 seconds.")
            plugin.server.scheduler.runTaskLater(plugin, { _ -> key.quota = false }, 60 * 20)
        } else {
            plugin.logger.warning("OpenAI Request Failed: ${response.code} - $body")
        }
    }
}